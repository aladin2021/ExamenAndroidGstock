...
import static  com.example.examenandroidgstock.SharedHelper.sha256;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //si la table users est vide alors ajouter l'utilisateur admin avec mot de passe "199"
        SQLiteStatement s = db.compileStatement("select count(*) from users;");
        long c = s.simpleQueryForLong();
        if(c==0){
            db.execSQL("insert into users(login, password) values (?,?)",new String[]{"admin", sha256("199")});
        }
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    if(p.equals(sha256(strPwd))){
                        ...
                        Intent i = new Intent(getApplicationContext(),gstocksMainActivity.class);
                        startActivity(i);
                    }else {
                        _txtLogin.setText("");
